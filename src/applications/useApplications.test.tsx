...
describe('useApplications', () => {
  ...
  it('applies filter params to the request', async () => {
    await authenticate();
    render(<Harness params={{ status: 'interview' }} />);
  ...
  it('refetches when the params change', async () => {
    await authenticate();
    const { rerender } = render(<Harness params={{}} />);
    expect(await screen.findByText('Initech')).toBeInTheDocument();

    rerender(<Harness params={{ status: 'interview' }} />);
  ...
  it('exposes pagination from the response', async () => {
    await authenticate();
    render(<Harness params={{ pageSize: 2 }} />);
